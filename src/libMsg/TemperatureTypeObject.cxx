...
/*!
 * @file TemperatureTypeObject.cpp
...
void registerTemperatureTypes()
{
    static std::once_flag once_flag;
    std::call_once(once_flag, []()
            {
                TypeObjectFactory *factory = TypeObjectFactory::get_instance();
                factory->add_type_object("Temperature", GetTemperatureIdentifier(true),
                GetTemperatureObject(true));
                factory->add_type_object("Temperature", GetTemperatureIdentifier(false),
                GetTemperatureObject(false));
...
const TypeIdentifier* GetTemperatureIdentifier(bool complete)
{
    const TypeIdentifier * c_identifier = TypeObjectFactory::get_instance()->get_type_identifier("Temperature", complete);
    if (c_identifier != nullptr && (!complete || c_identifier->_d() == EK_COMPLETE))
    {
        return c_identifier;
    }

    GetTemperatureObject(complete); // Generated inside
    return TypeObjectFactory::get_instance()->get_type_identifier("Temperature", complete);
}

const TypeObject* GetTemperatureObject(bool complete)
{
    const TypeObject* c_type_object = TypeObjectFactory::get_instance()->get_type_object("Temperature", complete);
    ...
    {
        return GetCompleteTemperatureObject();
    }
    //else
    return GetMinimalTemperatureObject();
}

const TypeObject* GetMinimalTemperatureObject()
{
    const TypeObject* c_type_object = TypeObjectFactory::get_instance()->get_type_object("Temperature", false);
    if (c_type_object != nullptr)
    {
        return c_type_object;
    }
    ...
    MinimalStructMember mst_temperature;
    mst_temperature.common().member_id(memberId++);
    mst_temperature.common().member_flags().TRY_CONSTRUCT1(false); // Unsupported
    mst_temperature.common().member_flags().TRY_CONSTRUCT2(false); // Unsupported
    mst_temperature.common().member_flags().IS_EXTERNAL(false); // Unsupported
    mst_temperature.common().member_flags().IS_OPTIONAL(false);
    mst_temperature.common().member_flags().IS_MUST_UNDERSTAND(false);
    mst_temperature.common().member_flags().IS_KEY(false);
    mst_temperature.common().member_flags().IS_DEFAULT(false); // Doesn't apply
    mst_temperature.common().member_type_id(*TypeObjectFactory::get_instance()->get_type_identifier("int32_t", false));


    MD5 temperature_hash("temperature");
    for(int i = 0; i < 4; ++i)
    {
        mst_temperature.detail().name_hash()[i] = temperature_hash.digest[i];
    }
    type_object->minimal().struct_type().member_seq().emplace_back(mst_temperature);
    ...
    TypeObjectFactory::get_instance()->add_type_object("Temperature", &identifier, type_object);
    delete type_object;
    return TypeObjectFactory::get_instance()->get_type_object("Temperature", false);
}

const TypeObject* GetCompleteTemperatureObject()
{
    const TypeObject* c_type_object = TypeObjectFactory::get_instance()->get_type_object("Temperature", true);
    ...
    CompleteStructMember cst_temperature;
    cst_temperature.common().member_id(memberId++);
    cst_temperature.common().member_flags().TRY_CONSTRUCT1(false); // Unsupported
    cst_temperature.common().member_flags().TRY_CONSTRUCT2(false); // Unsupported
    cst_temperature.common().member_flags().IS_EXTERNAL(false); // Unsupported
    cst_temperature.common().member_flags().IS_OPTIONAL(false);
    cst_temperature.common().member_flags().IS_MUST_UNDERSTAND(false);
    cst_temperature.common().member_flags().IS_KEY(false);
    cst_temperature.common().member_flags().IS_DEFAULT(false); // Doesn't apply
    cst_temperature.common().member_type_id(*TypeObjectFactory::get_instance()->get_type_identifier("int32_t", false));


    cst_temperature.detail().name("temperature");

    type_object->complete().struct_type().member_seq().emplace_back(cst_temperature);
    ...
    // Header
    type_object->complete().struct_type().header().detail().type_name("Temperature");
    // TODO inheritance
    TypeIdentifier identifier;
    ...
    TypeObjectFactory::get_instance()->add_type_object("Temperature", &identifier, type_object);
    delete type_object;
    return TypeObjectFactory::get_instance()->get_type_object("Temperature", true);
}
