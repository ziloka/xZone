...
int main(int argc, char* argv[])
{
	...
	std::shared_ptr<CfgHygrometer> cfgHygrometer = std::make_shared<CfgHygrometer>(cfg->getHygrometer());
	HumidityPublisher mypub(mutex, cfgHygrometer);
	if (mypub.init(use_environment_qos))
	{
		std::thread subscriber(createUpdateHygrometerSubscriber, mutex, cfgHygrometer, use_environment_qos);
