...
#include "TemperatureSubscriber.h"
...
void TemperatureSubscriber::SubListener::on_subscription_matched(
    DataReader*,
...
void TemperatureSubscriber::SubListener::on_data_available(DataReader* reader)
{
    SampleInfo info;
    if (reader->take_next_sample(&temperature_, &info) == ReturnCode_t::RETCODE_OK)
    {
...
void TemperatureSubscriber::run(uint32_t number)
{
