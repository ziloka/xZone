...
#include "UpdateHygrometerPublisher.h"
...
UpdateHygrometerPublisher::UpdateHygrometerPublisher()
...
void UpdateHygrometerPublisher::PubListener::on_publication_matched(
...
void UpdateHygrometerPublisher::runThread(
    uint32_t samples,
    uint32_t sleep)
{
...
std::thread UpdateHygrometerPublisher::run(uint32_t samples, uint32_t sleep)
{
    stop_ = false;
    std::thread thread(&UpdateHygrometerPublisher::runThread, this, samples, sleep);
    if (samples == 0)
    {
...
bool UpdateHygrometerPublisher::publish(bool waitForListener)
{
